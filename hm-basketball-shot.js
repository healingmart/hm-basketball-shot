/*
 * HealingMart Basketball Shot v27 STANDALONE
 * Standalone GitHub Pages core.
...
(function(){
  ...
  function getRoot(){
    ...
    for(var i=0;i<nodes.length;i++){
      var el=nodes[i];
      var r;
      try{ r=el.getBoundingClientRect(); }catch(_){ r={width:0,height:0}; }
      var visible=false;
      try{
        var cs=window.getComputedStyle(el);
        visible=cs.display!=='none' && cs.visibility!=='hidden' && Number(cs.opacity||1)!==0 && r.width>40 && r.height>40;
      }catch(_){ visible=r.width>40 && r.height>40; }
      var score=(visible?1000000000:0)+(r.width*r.height);
  ...
  function threeRevision(){
    ...
    return String(window.THREE.REVISION||'').trim();
  }

  function hasUsableThree(){
    var T=window.THREE;
    if(!T || !T.WebGLRenderer || !T.Scene || !T.Vector3 || !T.Mesh) return false;
    var rev=parseInt(T.REVISION,10);
    if(Number.isFinite(rev) && rev<149) return false;
    return true;
  }
  ...
  function start(){
    ...
    if(!hasUsableThree()){
      ...
      console.error('[Basketball Shot] compatible THREE.js did not load. Current revision:', threeRevision()||'none');
      return;
    }
    root.dataset.hmThreeRevision=threeRevision()||'unknown';
    started=true;
    try{
      (() => {
        'use strict';
  ...
  function waitForThree(maxMs) {
    return new Promise(function(resolve){
      ...
      (function check(){
        if (hasUsableThree()) { resolve(true); return; }
  ...
  function loadScriptWithTimeout(url, marker, timeoutMs) {
    return new Promise(function(resolve){
      ...
      if (hasUsableThree()) { done(true); return; }
      ...
      if (existing) {
        ...
        timer = setTimeout(function(){ done(hasUsableThree()); }, timeoutMs);
        return;
      }
  ...
  async function boot() {
    ...
    console.error('[Basketball Shot] compatible THREE.js failed from all CDN sources. Current revision:', threeRevision()||'none');
  }

  window.HM_BASKETBALL_CORE_VERSION='27';
  window.HMBasketballShotBoot=boot;
